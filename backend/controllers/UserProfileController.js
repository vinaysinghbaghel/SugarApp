...
exports.createUserProfile = function(req, res, next) {
    try {
      ...
      UserProfile.findOne({'email':req.query.email},function(err, user) {
        if (err) {
                return res.json({
     ...
     if(req.busboy!=null){
       ...
       let custID = randomString({length: 13});
       let uuids = uuid();
...
exports.usersignin = function(req, res){ 
  passport.authenticate('local', function (err, user, info) {
    ...
    if (err) {
      return res.json({success : false, "message" : "Invalid Username Or Password",user:{}});
    }
    if (!user) {
      return res.json({success : false, "message" : "Invalid Username Or Password",user:{}});
    } 
    ...
    return res.json({success : true,"message":"User Successfully Login",user:user});
  })(req, res);
};
